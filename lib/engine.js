...
// Per-player piece supply, as in the classic game. Upgrading a settlement
// to a city returns the settlement piece to the supply.
export const PIECE_LIMITS = { road: 15, settlement: 5, city: 4 };

export function pieceCounts(state, pid) {
  let roads = 0;
  let settlements = 0;
  let cities = 0;
  for (const p of Object.values(state.roads)) if (p === pid) roads++;
...
export function legalRoads(state, geom, pid) {
  if (pieceCounts(state, pid).roads >= PIECE_LIMITS.road) return [];
  const out = [];
...
export function legalSettlements(state, geom, pid) {
  if (pieceCounts(state, pid).settlements >= PIECE_LIMITS.settlement) return [];
...
// Bank trade rates per resource: 4 by default, 3 with a generic harbor,
// 2 with the matching resource harbor.
export function tradeRates(state, pid) {
  const rates = { wood: 4, clay: 4, wool: 4, grain: 4, ore: 4 };
  let generic = 4;
  for (const port of state.board.ports) {
    const owned = port.verts.some((v) => state.buildings[v] && state.buildings[v].player === pid);
    if (!owned) continue;
    if (port.kind === 'any') generic = 3;
    else rates[port.kind] = 2;
  }
  for (const r of RESOURCES) rates[r] = Math.min(rates[r], generic);
  return rates;
}

export function legalCities(state, pid) {
  if (pieceCounts(state, pid).cities >= PIECE_LIMITS.city) return [];
...
export function reduce(prev, geom, action) {
  ...
  switch (action.type) {
    ...
    case 'BUILD_CITY': {
      ...
      if (!legalCities(state, state.current).includes(action.vid)) return prev;
      pay(me, COSTS.city);
      state.buildings[action.vid].type = 'city';
    ...
    case 'TRADE_BANK': {
      if (state.phase !== 'main') return prev;
      const rate = tradeRates(state, state.current)[action.give];
      if (action.give === action.get || me.resources[action.give] < rate) return prev;
      me.resources[action.give] -= rate;
      me.resources[action.get]++;
      log(
        state,
        `${me.name} trades ${rate} ${RESOURCE_INFO[action.give].name} for 1 ${RESOURCE_INFO[action.get].name}${rate < 4 ? ' at the harbor' : ''}.`
      );
      return state;
    }
